 ...
 */
namespace Otomaties\Core\Carbon\Laravel;

use Otomaties\Core\Carbon\Carbon;
use Otomaties\Core\Carbon\CarbonImmutable;
use Otomaties\Core\Carbon\CarbonInterval;
use Otomaties\Core\Carbon\CarbonPeriod;
use Otomaties\Core\Illuminate\Contracts\Events\Dispatcher as DispatcherContract;
use Otomaties\Core\Illuminate\Events\Dispatcher;
use Otomaties\Core\Illuminate\Events\EventDispatcher;
use Otomaties\Core\Illuminate\Support\Carbon as IlluminateCarbon;
use Otomaties\Core\Illuminate\Support\Facades\Date;
use Throwable;
class ServiceProvider extends \Otomaties\Core\Illuminate\Support\ServiceProvider
{
    /** @var callable|null */
    ...
    public function boot()
    {
        ...
        if ($this->isEventDispatcher($events)) {
            $events->listen(class_exists('Otomaties\Core\Illuminate\Foundation\Events\LocaleUpdated') ? 'Illuminate\Foundation\Events\LocaleUpdated' : 'locale.changed', function () use ($service) {
    ...
    {
        return \function_exists('Otomaties\Core\app') ? \Otomaties\Core\app(...$args) : null;
    }
