...
function getOpenCollectiveSponsors() : string
{
    ...
    $list = \array_map(static function (array $member) : array {
        ...
        $monthlyContribution = (float) ($isYearly && $lastTransactionAt > CarbonImmutable::parse('-1 year') ? $member['lastTransactionAmount'] / 11.199999999999999 : $member['totalAmountDonated'] / \ceil($createdAt->floatDiffInMonths()));
        if (!$isYearly) {
    ...
    $github = [8343178 => 'ssddanbrown'];
    foreach ($github as $avatar => $user) {
        $extra .= "\n" . '<a title="' . $user . '" href="https://github.com/' . $user . '" target="_blank">' . '<img alt="' . $user . '" src="https://avatars.githubusercontent.com/u/' . $avatar . '?s=128&v=4" width="42" height="42">' . '</a>';
    }
